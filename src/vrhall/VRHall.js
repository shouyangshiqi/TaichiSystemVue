...
export class VRHall {
  ...
  _initTransformControls() {
    ...
    this._transformControls.addEventListener("objectChange", (e) => {
      ...
      const { position, scale, rotation, quaternion } = obj;
      console.log(
    ...
    window.addEventListener("keydown", (e) => {
      ...
      e.key === "t" && this._transformControls.setMode("rotate");
  ...
  _animate() {
    ...
    if (this._renderer) {
      // 注意：渲染顺序可通过调整场景中对象的添加顺序或修改下面的渲染逻辑控制
      // 若要强制控制渲染顺序，可在此处添加对象排序逻辑
    ...
    return new Promise((resolve) => {
      ...
      this._gltfLoader.load(
        ...
        (gltf) => {
          ...
          // 注意：添加到场景的顺序会影响渲染顺序，可调整此处添加顺序控制渲染
   ...
   * 加载高斯点云模型
   * @param {Object} params
   * @param {string} params.url - ksplat文件路径
   * @param {Array} [params.position=[0,0,0]] - 位置 [x, y, z]
   * @param {Array} [params.rotation=[0,0,0,1]] - 四元数旋转 [x, y, z, w]
   * @param {Array} [params.scale=[1,1,1]] - 缩放 [x, y, z]
   * @param {number} [params.splatAlphaRemovalThreshold=5] - alpha阈值
   * @returns {Promise<GaussianSplats3D.DropInViewer>}
   */
  loadGaussianSplat({
    url,
    position,
    ...
    splatAlphaRemovalThreshold = 5,
      ...
      viewer.addSplatScene(url, {
        ...
        .then(() => {
          ...
          this.attachTransformControls(viewer);
        })
        .catch((error) => {
          console.error("高斯点云模型加载失败:", error);
        });
      }
